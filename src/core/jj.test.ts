...
describe("jj command helpers", () => {
  test("reports a friendly error when jj is not installed or not on PATH", () => {
    expect(() =>
      ...
      'Jujutsu is required for `dunk diff` when `vcs = "jj"`, but `definitely-not-a-real-jj-binary` was not found in PATH.',
    );
  });
  ...
  realJjTest("reports a friendly error outside a jj repository", () => {
    ...
    ).toThrow('`dunk diff` must be run inside a Jujutsu repository when `vcs = "jj"`.');
  });
  ...
  realJjTest("reports a friendly error for invalid revsets", () => {
    ...
    ).toThrow("`dunk diff missing_revision` could not resolve Jujutsu revset `missing_revision`.");
  });
  ...
  realJjTest("reports a friendly error for ambiguous change id prefixes", () => {
    ...
    ).toThrow(`\`dunk diff ${prefix}\` could not resolve Jujutsu revset \`${prefix}\`.`);
  });
});
