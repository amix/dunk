...
describe("hunk navigation", () => {
  ...
  test("moves forward across hunk and file boundaries", () => {
    expect(findNextHunkCursor(cursors, indexHunkCursors(cursors), "alpha", 0, 1)).toEqual({
      fileId: "alpha",
      hunkIndex: 1,
    });
    expect(findNextHunkCursor(cursors, indexHunkCursors(cursors), "alpha", 1, 1)).toEqual({
      fileId: "beta",
      hunkIndex: 0,
    });
  });
  ...
  test("moves backward across file boundaries", () => {
    expect(findNextHunkCursor(cursors, indexHunkCursors(cursors), "beta", 0, -1)).toEqual({
      fileId: "alpha",
      hunkIndex: 1,
    });
    expect(findNextHunkCursor(cursors, indexHunkCursors(cursors), "alpha", 1, -1)).toEqual({
  ...
  test("starts at the nearest stream edge when no current hunk is selected", () => {
    expect(findNextHunkCursor(cursors, indexHunkCursors(cursors), undefined, 0, 1)).toEqual({
      fileId: "alpha",
      hunkIndex: 0,
    });
    expect(findNextHunkCursor(cursors, indexHunkCursors(cursors), undefined, 0, -1)).toEqual({
...
describe("annotated hunk navigation", () => {
  ...
  test("only includes hunks that have overlapping annotations", () => {
    ...
    const fileA = createTestFile("alpha", "alpha.ts", beforeA, afterA, [
      { newRange: [17, 17], summary: "Note on hunk 1" },
    ]);
    const fileB = createTestFile("beta", "beta.ts", beforeB, afterB, [
      { newRange: [1, 1], summary: "Note on beta" },
    ]);
  ...
  test("skips files with agent context but no matching annotations", () => {
    ...
    const fileA = createTestFile("alpha", "alpha.ts", beforeA, afterA, [
      { newRange: [10, 10], summary: "Note in gap, no hunk overlap" },
    ]);
  ...
  test("navigates forward and backward through annotated cursors only", () => {
    ...
    const fileA = createTestFile("alpha", "alpha.ts", beforeA, afterA, [
      { newRange: [17, 17], summary: "Note on hunk 1 only" },
    ]);
    const fileB = createTestFile("beta", "beta.ts", beforeB, afterB, [
      { newRange: [1, 1], summary: "Note on beta" },
    ]);
    ...
    expect(
      findNextHunkCursor(annotatedCursors, indexHunkCursors(annotatedCursors), "alpha", 1, 1),
    ).toEqual({
      fileId: "beta",
      hunkIndex: 0,
    });
    ...
    expect(
      findNextHunkCursor(annotatedCursors, indexHunkCursors(annotatedCursors), "beta", 0, -1),
    ).toEqual({
      fileId: "alpha",
      hunkIndex: 1,
    });
  ...
  test("jumps from an unannotated hunk to the nearest annotated one", () => {
    ...
    expect(
      findNextHunkCursor(annotatedCursors, indexHunkCursors(annotatedCursors), "alpha", 0, 1),
    ).toEqual({
      fileId: "alpha",
      hunkIndex: 1,
    });
