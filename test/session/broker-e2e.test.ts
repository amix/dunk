...
const sessionBrokerE2eDescribe = enableSessionBrokerE2eTests ? describe : describe.skip;
...
sessionBrokerE2eDescribe("session broker end-to-end", () => {
  ...
  test("a normal Hunk session still renders and exits cleanly when a non-Hunk listener owns the MCP port", async () => {
    ...
    try {
      ...
      const transcript = stripTerminalControl(await Bun.file(fixture.transcript).text());
      expect(transcript).not.toContain("View  Navigate  Theme  Agent  Help");
